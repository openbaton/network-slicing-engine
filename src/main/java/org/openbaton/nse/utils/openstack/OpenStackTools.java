...
public class OpenStackTools {
  ...
  public List<? extends Port> getNeutronPorts(OSClient os) {
      ...
      for (org.openstack4j.model.identity.v3.Service s : service_list) {
        ...
        if (s.getName().equals("neutron")) {
          ...
          for (org.openstack4j.model.identity.v3.Endpoint e : e_list) {
            //logger.debug("    Found OpenStack Neutron endpoint with type : " + e.getIface().value());
  ...
  public Map<String, String> getComputeNodeMap(OSClient os) {
    Map<String, String> computenode_ip_map = new HashMap<>();
  ...
  public HashMap<String, ArrayList<String>> getPortIps(OSClient os, ArrayList<String> ips) {
    HashMap<String, ArrayList<String>> port_map = new HashMap<>();
